...
SYSTEM_PROMPT = """
You are KrishiSage — an expert agricultural assistant. 
Be concise, analytical, and practical. 
Focus on soil health, pests, and crop optimization.
"""
...
def _cached_completion(message_hash: str, user_message: str) -> str:
    ...
    for model in MODELS:
        try:
            ...
            last_err = str(e)
            print(f"Assistant: {model} failed, trying next... Error: {last_err[:50]}")
            time.sleep(1) # Tiny pause to reset rate-limit window
            continue
            
    raise Exception(f"All assistant models failed. Last error: {last_err}")
...
async def agri_assistant(req: AssistantRequest):
    try:
        ...
        print(f"FINAL ASSISTANT ERROR: {e}")
        return {"success": False, "response": "KrishiSage is currently busy. Please try again in a moment."}